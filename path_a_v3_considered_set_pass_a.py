...
FIXTURE = ROOT / "tests" / "fixtures" / "path_a_v3_considered_set_2026_07_16.json"
...
def _case_result(case: dict) -> dict:
    ...
    matched = (
        ...
        and result["missing_surface_ids"] == expected.get("missing_surface_ids", [])
    )
    return {
        "case_id": case["id"],
...
def build_artifact() -> dict:
    ...
    return {
        "artifact": "path_a_v3_considered_set_pass_a",
        "generated_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "zero_model": True,
        "implementation": "agents.considered_set_gate.evaluate_considered_set_case",
        "frozen_packet": {
            "packet": packet["packet"],
            "fixture_path": str(FIXTURE.relative_to(ROOT)),
            "prereg": packet["prereg"],
            "prereg_freeze_commit": packet["prereg_freeze_commit"],
            "fixture_freeze_commit": "38d3774",
...
def _write_markdown(artifact: dict, path: Path) -> None:
    lines = [
        ...
        "",
        (
            f"- `{artifact['frozen_packet']['packet']}` — "
            f"`{artifact['frozen_packet']['fixture_path']}` — "
            f"prereg freeze `{artifact['frozen_packet']['prereg_freeze_commit']}` — "
            f"fixture freeze `{artifact['frozen_packet']['fixture_freeze_commit']}`"
        ),
        "",
        "## Per-Case Results",
        "",
    ...
    for row in artifact["case_results"]:
        ...
        lines.extend(
            [
                ...
                f"- Expected missing surfaces: `{json.dumps(row['expected'].get('missing_surface_ids', []))}`",
                f"- Actual missing surfaces: `{json.dumps(actual['missing_surface_ids'])}`",
...
def main() -> None:
    ...
    json_path = ARTIFACT_DIR / f"path_a_v3_considered_set_pass_a_{stamp}.json"
    md_path = ARTIFACT_DIR / f"path_a_v3_considered_set_pass_a_{stamp}.md"
